...
setMethod(
  ...
  definition=function(Object, threads=1, simplify=FALSE) {
    ...
    if (simplify == TRUE) {
      message("CAUTION! The simplify parameter is set to TRUE. For robust results consider changing the simplify parameter to FALSE.")
      simplifyNumber <- 4
    } else {
      simplifyNumber <- 1
    }
    ...
    if (nimp == 1) { # conditional if only one or no imputation
      impgo <- 1
      ## Step 1. Data Complexity Reduction
        ...
        pb <- txtProgressBar(max=length(seq((length(possible_heights)-(ncol(Object@data)-2)), (length(possible_heights)-1), simplifyNumber)), style=3)
        progress <- function(n) setTxtProgressBar(pb, n)
        opts <- list(progress=progress)

        ### Step 1.2. Reprocessing
        foreach(heights_cut=seq((length(possible_heights)-(ncol(Object@data)-2)), (length(possible_heights)-1), simplifyNumber), .packages=packages.parallel, .export=functions.parallel, .options.snow=opts) %dopar% { # compute for all possible heights
    ...
    ### END OF one or no imputation conditional
    } else { # if more than one imputation
      ...
      foreach(impgo=1:nimp, .packages=packages.parallel, .export=functions.parallel, .options.snow=opts) %dopar% { # for every imputation
        ...
        ### Step 1.2. Reprocessing
        for (heights_cut in seq((length(possible_heights)-(ncol(Object@data)-2)), (length(possible_heights)-1), simplifyNumber)) { # compute for all possible heights
    ...
    ### END OF more than one imputation conditional
